...
dir <- "/Users/yufeng.lai/Documents/carbon_footprint/consumer_WTP_carbon_footprint"
setwd(dir)
...
The following section presents the principal component analysis (PCA) results for the climate attitude questions (Q10 and Q12). 
We use ordinal PCA (ordPCA) from the ordPens package, which handles ordinal variables by optimally scaling them before computing principal components.

### PCA for Q10 (Sustainable Behaviors)
Q10 measures the frequency of sustainable behaviors with 5-point scale (1 = Almost Always, 5 = Never).

```{r pca_q10, echo=FALSE, message = FALSE}
### Read demographics data for PCA
dt_demo <- fread("csv/dt_demo.csv")

### Principle component analysis for Q10
message("Running PCA for Q10...")
...
l_star_q10 <- lambda_seq[which.max(
...
### Calculate factor loadings as correlations between original variables and PCs
q10_mat <- as.matrix(dt_q10)
q10_loadings <- cor(q10_mat, pc_fit_q10_star$X, use = "complete.obs")
colnames(q10_loadings) <- c("PC1", "PC2")

### Display loadings table
q10_loadings_df <- data.frame(
    Variable = colnames(dt_q10),
    PC1 = round(q10_loadings[, 1], 3),
    PC2 = round(q10_loadings[, 2], 3)
)
q10_loadings_df
```

### PCA for Q12 (Climate Change Beliefs)
Q12 measures climate change beliefs with 5-point scale (1 = Strongly agree, 5 = Strongly disagree).

```{r pca_q12, echo=FALSE, message = FALSE}
### Principle component analysis for Q12
message("Running PCA for Q12...")
...
l_star_q12 <- lambda_seq[which.max(
...
### Calculate factor loadings as correlations between original variables and PCs
q12_mat <- as.matrix(dt_q12)
q12_loadings <- cor(q12_mat, pc_fit_q12_star$X, use = "complete.obs")
colnames(q12_loadings) <- c("PC1", "PC2")

### Display loadings table
q12_loadings_df <- data.frame(
    Variable = colnames(dt_q12),
    PC1 = round(q12_loadings[, 1], 3),
    PC2 = round(q12_loadings[, 2], 3)
)
q12_loadings_df
```
...
```{r pca_variance, echo=FALSE, message = FALSE}
### Calculate variance explained for Q10
q10_var_explained <- apply(pc_fit_q10_star$X, 2, var) / sum(apply(q10_mat, 2, var, na.rm = TRUE)) * 100

### Calculate variance explained for Q12
q12_var_explained <- apply(pc_fit_q12_star$X, 2, var) / sum(apply(q12_mat, 2, var, na.rm = TRUE)) * 100

variance_df <- data.frame(
    Question = c("Q10", "Q10", "Q12", "Q12"),
    Component = c("PC1", "PC2", "PC1", "PC2"),
    `Variance Explained (%)` = round(c(q10_var_explained, q12_var_explained), 2),
    check.names = FALSE
)
variance_df
```
...
```{r pca_q12_pc1, echo=FALSE, message = FALSE}
message("Fitting model using first principal component of Q12...")
ict_fit_pca_q12_pc1 <-
    ictreg(
        count ~ as.factor(list_id) +
            Q12_PC1,
...
make_summary(ict_fit_pca_q12_pc1)
```

### Climate Attitudes, First Principal Component of Q10
```{r pca_q10_pc1, echo=FALSE, message = FALSE}
message("Fitting model using first principal component of Q10...")
ict_fit_pca_q10_pc1 <-
    ictreg(
        count ~ as.factor(list_id) +
            Q10_PC1,
...
make_summary(ict_fit_pca_q10_pc1)
```

### Climate Attitudes, First Principal Components of Q12 and Q10
```{r pca_q12_q10_pc1, echo=FALSE, message = FALSE}
message("Fitting model using first principal components of Q12 and Q10...")
ict_fit_pca_q12_q10_pc1 <-
    ictreg(
        count ~ as.factor(list_id) +
            Q12_PC1 +
            Q10_PC1,
...
make_summary(ict_fit_pca_q12_q10_pc1)
```

### Combined Model: First Principal Components of Q12, Q10 and Demographics
```{r pca_q12_q10_demo, echo=FALSE, message = FALSE}
message("Fitting model using first principal components of Q12, Q10 and demographics...")
ict_fit_pca_q12_q10_demo <-
    ictreg(
        count ~ as.factor(list_id) +
            Q12_PC1 +
            Q10_PC1 +
...
make_summary(ict_fit_pca_q12_q10_demo)
```
